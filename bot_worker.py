...
ADMIN_ID = 5163892491
# Correctly get PANEL_API_URL from environment variable set by render.yaml
PANEL_API_URL = os.environ.get("PANEL_API_URL", "http://127.0.0.1:10000")
...
def start(update: Update, context: CallbackContext):
    ...
    if user.id == ADMIN_ID:
        ...
        if not any("Go to Web Panel" in btn.text for row in kbd for btn in row):
            public_panel_url = PANEL_API_URL.replace(":10000", "")
            kbd.append([InlineKeyboardButton("🌐 Go to Web Panel", url=public_panel_url)])
