...
# Define the data directory path. This is safe for both local and Render deployments.
DATA_DIR = os.path.join(os.getcwd(), 'data')

# Create the data directory if it doesn't exist
os.makedirs(DATA_DIR, exist_ok=True)

# Set the database URI to be inside the DATA_DIR
...
def _secure_join(base_folder, relative_path):
    ...
    if not os.path.abspath(final_path).startswith(os.path.abspath(full_base_path)): return None
    return final_path
...
def get_system_stats_dict():
    ...
    try:
        ...
        try:
            ...
            app.logger.warning(f"Could not get Disk stats: {e}")
...
if __name__ == '__main__':
    all_dirs_to_create = DATABASE_FOLDERS + [PRIVATE_DATABASE_FOLDER, CHECKER_RESULTS_PATH, INFO_DATABASE_FOLDER, ADMIN_DATABASE_FOLDER]
    for folder in all_dirs_to_create:
